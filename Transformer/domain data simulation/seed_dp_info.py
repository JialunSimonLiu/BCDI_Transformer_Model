...
N = 10 # Number of crystals to generate
plot_crystal_indices = [0]

#################### Parameters ####################
grid_size = 32  # Size of the crystal
pad_size = 64  # Size of the sample
zoom_size = 64  # Size of the zoomed diffraction pattern
...
real_amp_2d = np.zeros((N, 64, 64))
real_pha_2d = np.zeros((N, 64, 64))
real_density_2d = np.zeros((N, 64, 64)).astype(np.complex128)
dp_2d = np.zeros((N, 64, 64))
dp_norm_2d = np.zeros((N, 64, 64))
seeds_dp= []
...
def transform_diffraction_pattern(dp_pattern):
    ...
    x, y = np.meshgrid(np.arange(64), np.arange(64), indexing='xy')
    x_flat = x.flatten()
    y_flat = y.flatten()

    # Initialize the final array
    final_dp = np.zeros((4096, 3))
...
for crystal_index in range(N):
    ...
    seeds = generate_circular_seeds(num_seeds, grid_size // 2)
    ...
    seeds_pos_phase = np.hstack((seeds_pad_pos, phase[:, np.newaxis]))
...
def save_data(crystals, file_path='data_seeds_dp.h5'):
...
def save_data_as_text(crystals, file_path='data.txt'):
