...
def custom_loss(output, targets, grid_size, radius, sigma = sigma, pad_size = pad_size):
    ...
    output[:, :, 0:2] = output[:, :, 0:2].round().int()
    # Avoid seed overlap
    for batch_idx in range(batch_size):
        output[batch_idx] = avoid_seed_overlap(output[batch_idx], min_distance=1.0)
    #output = avoid_seed_overlap(output, min_distance=1.0)
    ...
    for batch_idx in range(batch_size):
        ...
        temp_real_space = create_density_function(1.0, phase_pred)
    ...
    phase_pred_np = phase_pred.detach().cpu().numpy()
    print(seeds_pred)
    """plt.imshow(phase_pred_np, cmap='viridis')
    plt.colorbar()
    ...
    #print(predicted_dp.requires_grad)
    #print(target_dp.requires_grad)
